...
const CoachCard: React.FC<CoachCardProps> = ({
  ...
  const renderSocialLinks = () => {
    ...
    const platforms = Object.entries(coach.socialMedia).filter(
      ([_, url]) => url
    );
  ...
  const renderBadges = () => {
    ...
    if (coach.certifications && coach.certifications.length > 0) {
      badges.push(
        <span
          key="certified"
          className={`${styles.badge} ${styles.badgeCertified}`}
        >
          {t("certified")}
        </span>
      );
    ...
    if (coach.portfolio && coach.portfolio.length > 0) {
      badges.push(
        <span
          key="portfolio"
          className={`${styles.badge} ${styles.badgePortfolio}`}
        >
          {t("hasPortfolio")}
        </span>
      );
    ...
    return (
      <div className={styles.rating}>
        <span className={styles.stars}>
          {"★".repeat(Math.round(coach.averageRating))}
        </span>
        ...
        <div className={styles.cardHeader}>
          ...
          <div className={styles.pricing}>
            <span className={styles.rate}>
              {formatRate(
                coach.rateAmount ?? coach.hourlyRate,
                coach.rateType ?? "HOUR"
              )}
            </span>
          </div>
    ...
    return (
      <div className={`${styles.card} ${styles.cardAdmin}`}>
        ...
        <div className={styles.adminMeta}>
          ...
          {coach.experience && (
            <div className={styles.metaItem}>
              <strong>{t("experience")}:</strong> {coach.experience}{" "}
              {t("years")}
            </div>
          )}
          {(coach.rateAmount ?? coach.hourlyRate) && (
        ...
        {coach.portfolio && coach.portfolio.length > 0 && (
          <div className={styles.portfolio}>
            <strong>{t("portfolio")}:</strong> {coach.portfolio.length}{" "}
            {t("items")}
          </div>
        )}
