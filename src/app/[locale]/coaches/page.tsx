...
export default function CoachesPage() {
  ...
  useEffect(() => {
    fetchCoaches();
  }, [selectedDiscipline, minRating, maxHourlyRate]);
      ...
      <div className={styles.mainContent}>
        ...
        <aside className={styles.sidebar}>
          <div className={styles.sidebarHeader}>
            ...
            <button type="button" onClick={handleResetFilters} className={styles.resetButton} aria-label={t('reset')}>
              {t('reset')}
            </button>
          </div>
          ...
          <div className={styles.filterGroup}>
            <label htmlFor="discipline-filter" className={styles.filterLabel}>{t('discipline')}</label>
            <select
          ...
          {/* Rating Filter */}
          <div className={styles.filterGroup}>
            <label htmlFor="rating-filter" className={styles.filterLabel}>{t('minimumRating')}</label>
            <select
              id="rating-filter"
              value={minRating}
          ...
          <div className={styles.filterGroup}>
            <label htmlFor="rate-filter" className={styles.filterLabel}>{t('maxHourlyRate')}</label>
            <input
              id="rate-filter"
              type="number"
              value={maxHourlyRate}
          ...
          {/* Apply Button */}
          <Button variant="primary" fullWidth onClick={() => fetchCoaches()}>
            {t('applyFilters')}
          </Button>
        </aside>
                ...
                <div key={coach.id} className={styles.coachCard}>
                  ...
                  <div className={styles.coachDetails}>
                      ...
                      {coach.hourlyRate && (
                        <div className={styles.coachRate}>
                          <span className={styles.rateAmount}>
                            {new Intl.NumberFormat('fr-FR').format(Number(coach.hourlyRate))} XAF
                          </span>
                    ...
                    <div className={styles.coachMeta}>
                      {(coach.city || coach.country) && (
                        <div className={styles.metaItem}>
                          ...
                          <span>{[coach.city, coach.country].filter(Boolean).join(', ')}</span>
                        </div>
                      )}
                      {coach.experienceYears && coach.experienceYears > 0 && (
                        <div className={styles.metaItem}>
                          ...
                          <span>{coach.experienceYears} {t('yearsExperience')}</span>
                        </div>
                      )}
                    ...
                    <div className={styles.coachFooter}>
                      {(coach.instagram || coach.facebook || coach.tiktok || coach.twitter || coach.youtube) && (
                        <div className={styles.socialLinks}>
                          {coach.instagram && (
                            <a href={coach.instagram} target="_blank" rel="noopener noreferrer" className={styles.socialLink} aria-label="Instagram">
                          ...
                          {coach.facebook && (
                            <a href={coach.facebook} target="_blank" rel="noopener noreferrer" className={styles.socialLink} aria-label="Facebook">
                              <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                          ...
                          {coach.tiktok && (
                            <a href={coach.tiktok} target="_blank" rel="noopener noreferrer" className={styles.socialLink} aria-label="TikTok">
                              <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                          ...
                          {coach.twitter && (
                            <a href={coach.twitter} target="_blank" rel="noopener noreferrer" className={styles.socialLink} aria-label="X (Twitter)">
                              <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                          ...
                          {coach.youtube && (
                            <a href={coach.youtube} target="_blank" rel="noopener noreferrer" className={styles.socialLink} aria-label="YouTube">
                              <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
